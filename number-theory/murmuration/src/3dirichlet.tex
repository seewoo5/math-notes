...
Note that Baker, Harman, and Pintz proved an unconditional result for the existence of a prime in a slightly longer short interval $[X, X + X^{0.525}]$, which means that Theorem \ref{thm:lop_dirichlet} holds with $\gamma \ge 0.525$ unconditionally \cite{baker2001difference}.
...
Note that $A$ will not be a fixed constant, but rather grows together with $X$: in fact, the proof uses $0 < \epsilon < (\gamma - \frac{3}{4}) / 5$ and $A = X^{1 + 5 \epsilon - \gamma}$.
To estimate the sum of the characters, one uses the Pólya--Vinogradov inequality: for a non-principal character $\chi \Mod q$,
\begin{equation}
    \label{eqn:pv_basic}
    \left|\sum_{M < n \le M + N} \chi(n)\right| \ll q^{\frac{1}{2}} \log q
\end{equation}
Using this, by assuming GRH, one can show that the summation over primes in the short interval $[yX, yX + X^\gamma]$ is bounded by (see \cite[eq. (5.1)]{granville2007large})
\begin{equation}
    \label{eqn:pv}
    \left|\sum_{\substack{p \in [yX, yX + X^\gamma] \\ p\text{ prime}}} \chi_d(p)\right| \ll (yX)^{\frac{1}{2} + \epsilon}
\end{equation}
for non-principal $\chi_d$ and $\frac{1}{2} < \gamma < 1$.
Combined with Abel's summation formula, \eqref{eqn:pv} shows that the remainder term $R_{\Phi, A}(y, X, \gamma)$ vanishes as $X \to \infty$, where $\gamma > \frac{3}{4}$ is used in the computation.
...
See \cite[Section 6.2]{lee2025murmurations} for an analogous result for $\chi_d$.
