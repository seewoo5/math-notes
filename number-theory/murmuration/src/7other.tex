...
Wang \cite{wang2025murmurations} computed the murmuration density for (possibly trivial) Hecke characters of imaginary quadratic fields.
...
The completed $L$-function $\Lambda(s, \psi)$ satisfies the functional equation $\Lambda(1 - s, \psi) = \Lambda(s, \psi)$, so the root number is always $+1$.
...
\begin{theorem}[Wang \cite{wang2025murmurations}]
    ...
    \end{align*}
\end{theorem}
See \cite[Theorem 1]{wang2025murmurations} for details.
These characters give rise to modular forms of weight 1 via theta series, and it is conjectured that almost all weight 1 modular forms arise this way \cite{frohlich1977algebraic}.
Note that the main term of \eqref{eqn:wangdensity} depends on the arithmetic of $p$.
However, the dependence on $p$ is explicit and $c(p) \delta_m(p)$ is almost periodic in $m$, a phenomenon that does not appear in other families.
In particular, he proved that taking a local average over short intervals of $p$ removes the dependence on $p$.
Especially, taking a local average in an interval of length $H$ so that the primes are equidistributed in $[X, X + H]$, in the sense that
\[
...
The main ingredients of the proof are orthogonality of characters and summation of class numbers in short intervals with the class number formula, similar to Zubrilina's approach.

\subsection{Flying modular forms (in weight direction)}


Recall that Zubrilina computed the murmuration density for a \emph{fixed weight $k$ and varying level $N$}.
...
In this case, they considered the family of Hecke newforms whose \emph{analytic conductor}
\[
...
\begin{theorem}[{Bober--Booker--Lee--Lowry-Duda \cite[Theorem 1.1]{bober2023murmurations}}]
    ...
    \begin{equation}
        \label{eqn:nuE}
        \nu(E) = \frac{1}{\zeta(2)} \sideset{}{^{*}}\sum_{(a/q)^{-2} \in E} \frac{\mu(q)^2}{\varphi(q)^2 \sigma(q)} \left(\frac{a}{q}\right)^{-3}
...
The main tool for the proof is the (original) Eichler--Selberg trace formula, which does not include Atkin--Lehner operators (e.g. \cite[Theorem 2.1]{child2022twist}).
Then, they apply the class number formula to replace class numbers with the special values of Dirichlet $L$-functions at $s = 1$, which can be estimated under GRH.
...
They considered a family of Maass forms where the spectral parameter ($R$ with $\lambda = \frac{1}{4} + R^2$) goes to $\infty$, which is equivalent to the \emph{analytic conductor} 
\[
\cN(R) := \frac{\exp\left(\psi\left(\frac{1/2 + a + iR}{2}\right)\right) + \exp \left(\psi \left(\frac{1/2 + a - iR}{2}\right)\right)}{\pi^2} \approx \frac{R^2}{4 \pi^2} + O(1)
\]
going to $\infty$.
Here $a = 0$ (resp. $a = 1$) if the Maass form is even (resp. odd).
...
\begin{theorem}[{Booker--Lee--Lowry-Duda--Seymour-Howell--Zubrilina \cite[Theorem 1.1]{booker2024murmurations}}]
    ...
    Assuming GRH for $L$-functions of Dirichlet characters and Maass forms, as $R \to \infty$ we have
    \begin{equation}
        \frac{\sum_{\substack{p\text{ prime} \\ p / N \in E}} \log p \sum_{|r(f) - R| \le H} \epsilon(f) a_{f}(p)}{\sum_{\substack{p\text{ prime} \\ p / N \in E}} \log p \sum_{|r(f) - R| \le H} 1} \to \frac{(-1)^\delta}{\sqrt{N}} \left(\frac{\nu(E)}{|E|} + o_{E, \epsilon}(1)\right)
    \end{equation}
    where $\nu(E)$ is the same as \eqref{eqn:nuE} of Theorem \ref{thm:bober-lee-ld}.
\end{theorem}
...
The remainder of the proof is similar to the weight aspect case of the modular forms \cite{bober2023murmurations}.
