...
The first murmuration density that was ever computed is for modular forms by Zubrilina \cite{zubrilina2025murmurations}.
...
Before we state the result, we first define some notation.
For each $k$ and $N \ge 1$, let $H^{\mathrm{new}}(N, k)$ be the set of normalized Hecke cusp forms of weight $k$ and level $\Gamma_0(N)$.
...
For $n \in \bZ_{\ge 0}$, the \emph{$n$-th Chebyshev polynomial of the second kind} is defined as
\[
...
\subsection{Skoruppa--Zagier trace formula}

To prove Theorem \ref{thm:zubrilina_modform}, one needs to understand how to estimate the numerator on the LHS.
...
Thus, the sum appearing in the numerator on the LHS of \eqref{eqn:zubrilina_modform} can be interpreted as the trace of the operator $(-1)^{k/2} T_P \circ W_N$ acting on the space of cusp forms of weight $k$ and level $N$ (multiplied by $P^{1 - k/2}$).
Eichler \cite{eichler1955class} studied such sums of traces and proved that they can be expressed in terms of (Hurwitz) class numbers, which was generalized by Selberg \cite{selberg1956harmonic}.
To account for the root number $\varepsilon(f)$, i.e., the eigenvalue of $W_N$, we use the following version of the Eichler--Selberg trace formula by Skoruppa and Zagier \cite{skoruppa1988jacobi}.

\begin{theorem}[Skoruppa--Zagier \cite{skoruppa1988jacobi}]
    \label{thm:sz_tf}
    For square-free $N$ and prime $P \nmid N$,
    \begin{equation}
        \label{eqn:sz_tf}
        \sum_{f \in H^\new(N, k)} \sqrt{P} \lambda_f(P) \varepsilon(f)\nonumber = \frac{H_1(-4PN)}{2} + (-1)^{k/2 - 1} U_{k - 2} \left(\frac{r\sqrt{N}}{2\sqrt{P}}\right) \sum_{0 < r \le 2\sqrt{P/N}} H_1(r^2 N^2 - 4PN) - \delta_{k=2}(P+1)
    \end{equation}
...
If we denote the spaces of eigenforms with root number $+1$ and $-1$ by $H^\new_+(N, k)$ and $H^\new_-(N, k)$ respectively, then the above theorem can be interpreted as the difference of the traces of $T_P$ acting on $H^\new_+(N, k)$ and $H^\new_-(N, k)$, while the Eichler--Selberg trace formula gives the sum of the two traces.
Theorem \ref{thm:sz_tf} is proven by using the theory of Jacobi forms, which we will not discuss here.
...
Then the sum (average) of the corresponding $L$-values can be estimated via truncation and Polya--Vinogradov inequality \eqref{eqn:pv_basic}.
...
\]
With some hard analysis, one gets the following estimations on the sum of $h(-PN)$ and $h(-4PN)$.
...
By integrating it in a suitable sense, one can also get the average over ``geometric intervals'' $[X, cX]$ for some constant $c > 1$.
...
\begin{theorem}[{\cite[Theorem 2]{zubrilina2025murmurations}}]
    ...
    \begin{equation*}
        ...
        \end{cases}
    \end{equation*}
    where $a = \frac{4}{9}(2^{3/2} - 1)\beta, b = \frac{2}{3}\gamma, c = \frac{2}{3}\alpha$.
...
The murmuration density function $\cM_k(y)$ has many interesting properties.
...
\emph{No!}
The reason is that elliptic curves over $\bQ$ correspond to modular forms of weight $2$ \emph{with coefficient field (Hecke field) $\bQ$} (recall that $a_p(E) = p + 1 - \#E(\bF_p)$ are integers).
The family (see also Section \ref{sec:general}) that Zubrilina considered is much larger than the family of elliptic curves in \cite{he2024murmurations}, and it seems hard to isolate such a family from the whole family of Hecke eigenforms (of weight 2).
...
\subsection{Without root number}

In \cite{martin2025distribution,martin2025variations}, Martin considered murmurations without the root number weight but with $\sqrt{N}$ scaling, i.e. the average of the form
\begin{equation}
\label{eqn:martin_avg}    
\bE_{\substack{N \in [X, cX] \\ N \text{ squarefree} \\ f \in H^{\text{new}}(N, k)}}[\sqrt{NP} \lambda_f(P)] = \frac{\sum_{N \in [X, cX]}^\square \sum_{f \in H^{\new}(N, k)} \sqrt{NP} \lambda_f(P)}{\sum_{N \in [X, cX]}^\square \sum_{f \in H^{\new}(N, k)} 1}
\end{equation}
or more generally, 
\begin{equation}
\label{eqn:martin_avg_wq}
\bE_{\substack{N \in [X, cX]  \\ f \in H^{\text{new}}(N, k)}}\left[w_Q(f) \sqrt{\frac{NP}{Q}} \lambda_f(P)\right]
\end{equation}
for a sequence of divisors $Q \mid N$ for each $N$ appearing in the average, where $w_Q(f)$ is the eigenvalue of the Atkin--Lehner involution $W_Q$ acting on $f$.
$Q = N$ corresponds to the Zubrilina's case (Theorem \ref{thm:zubrilina_geom}), while $Q = 1$ reduces to \eqref{eqn:martin_avg}.
He conjectured that we do have murmurations in such general cases for ``arithmetically compatible sequences of $(N, Q)$'' \cite[Conjecture 1.8]{martin2025distribution}, and computed the murmuration density for small $y = P / X$ (See also Figure \ref{fig:martin_wo_root}).
The analogous trace formula for $T_P \circ W_Q$ is given in \cite[Proposition 5.5]{martin2025distribution}, although the analysis would be much harder since the number of class number terms grows with $P$ (it is bounded in \eqref{eqn:sz_tf}).

\begin{figure}
    \centering
    \includegraphics[width=\textwidth]{src/martin_wo_root.png}
    \caption{Murmuration of modular forms without root number weight for $k = 2$, $N \in [1000, 2000]$ and $P < 4000$.}
    \label{fig:martin_wo_root}
\end{figure}