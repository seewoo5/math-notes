...
\begin{proof}
In order to prove this theorem, we must use a polarization for which the usual subgroups of $\PGL_2(\bA)$ and $\widetilde{\SL}_2(\bA)$ lie inside $P$.
...
\begin{align*}
    f(1) &= \int_{\SL_2(k) \backslash \SL_2(\bA)} \phi_1(0) F_2(h) \varphi(h) \dd h + \int_{N(k) \backslash \SL_2(\bA)} \phi_1(y_2 h) F_2(h) \varphi(h) \dd h.
\end{align*}
...
\]
Thus, if $\theta(\sigma, \psi^{-1}) \neq 0$, then there exists a $t$ for which $\varphi_{\psi^{t^2}}$ is non-zero.
...
\begin{align*}
    ...
    &= \int_{N(\bA) \backslash \SL_2(\bA)} \omega_\psi(1, h) \phi(y_2, t)  \varphi_{\psi^{t^2}}(h) \dd h.
\end{align*}
...
For $n \in N$
\[
\omega_\psi(1, n) \phi(y_2, t) = \psi(tn) \phi(y_2, t).
\]
